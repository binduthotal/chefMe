...
const Recipes = ({ user, filterFavorites = false }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(recipesRef, (snapshot) => {
      ...
      if (data) {
        const recipeArray = Object.values(data).sort(
  ...
  const handleShare = async (e, recipe) => {
    ...
    const ingredientText = recipe.ingredients
      ?.map(
        (i) => `- ${i.quantity} ${i.unit !== "piece" ? i.unit : ""} ${i.name}`,
      )
      .join("\n");
    ...
    if (navigator.share) {
      ...
      if (shareChoice === "1")
        ...
        window.open(
          `https://mail.google.com/mail/?view=cm&fs=1&su=${encodeURIComponent(subject)}&body=${encodeURIComponent(shareText)}`,
          "_blank",
        );
  ...
  const deleteRecipe = async (e, recipe) => {
    e.preventDefault();
  ...
  if (selectedRecipe) {
    return (
      <div className="mt-6 bg-white rounded-[28px] border border-[#628141]/10 p-6 sm:p-8 shadow-xl shadow-[#628141]/5 animate-in fade-in zoom-in-95 duration-200">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="p-2.5 bg-[#EBD5AB]/30 hover:bg-[#EBD5AB]/60 rounded-xl text-[#628141] transition"
              title="Share"
            >
              <svg
                className="w-5 h-5"
            ...
            <button
              ...
              className="p-2.5 bg-[#EBD5AB]/30 hover:bg-[#EBD5AB]/60 rounded-xl transition"
              title="Favorite"
            >
              <svg
                className={`w-5 h-5 ${selectedRecipe.isFavorite ? "fill-amber-400 text-amber-400" : "fill-none text-[#628141]/50"}`}
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
                <path
        ...
        <h2 className="text-3xl sm:text-4xl font-black text-[#1B211A] mb-4">
        ...
        <span className="px-10 text-[#628141] font-bold ">Cook for 2 to 3</span>

        {selectedRecipe.story && (
          <div className="bg-[#EBD5AB]/30 p-5 rounded-2xl border border-[#628141]/10 mb-8">
            <p className="text-[#1B211A]/80 italic leading-relaxed">
              "{selectedRecipe.story}"
            </p>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 md:gap-12">
          <div className="lg:col-span-1">
            <h3 className="text-xl font-bold text-[#1B211A] mb-4 border-b border-[#1B211A]/10 pb-3">
              Ingredients
            </h3>
            ...
            {/* <ul className="space-y-2">
              {selectedRecipe.ingredients?.map((ing, idx) => (
                <li
                  key={idx}
                  className="flex items-start gap-3 text-[#1B211A]/80 bg-white p-2.5 rounded-lg border border-[#628141]/10 shadow-sm"
                >
                  <span className="text-[#8BAE66] mt-0.5 font-bold">•</span>
                  <span>
                    <span className="font-black text-[#628141]">
                      {ing.quantity * scale}{" "}
                      {ing.unit !== "piece" ? ing.unit : ""}
                    </span>{" "}
                    <span className="font-medium text-[#1B211A]">
                      {ing.name}
                    </span>
                  </span>
                </li>
            ...
            <ul className="space-y-3">
              {selectedRecipe.ingredients?.map((ing, idx) => (
                <li
                  key={idx}
                  className="flex items-start gap-3 text-[#1B211A]/80 bg-white p-2 rounded-lg border border-[#628141]/10 shadow-sm"
                >
                  <span className="text-[#8BAE66] mt-0.5 font-bold">•</span>
                  <span>
  ...
  return (
    <div className="mt-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {displayedRecipes.map((recipe) => (
        <div
          key={recipe.id}
          className="group flex flex-col bg-white rounded-[24px] border border-[#628141]/20 shadow-sm hover:shadow-xl hover:shadow-[#628141]/10 hover:-translate-y-1 transition-all duration-300 overflow-hidden"
        >
          <div className="p-5 border-b border-[#1B211A]/5 flex-1 relative">
            <div className="absolute top-4 right-4 flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity bg-white/90 backdrop-blur-sm p-1.5 rounded-xl border border-[#628141]/10 shadow-sm">
              <button
                ...
                className="cursor-pointer p-1.5 hover:bg-[#EBD5AB]/50 rounded-lg text-[#628141] transition"
                title="Share"
              >
                <svg
                  className="w-4 h-4"
              ...
              <button
                ...
                className="cursor-pointer p-1.5 hover:bg-[#EBD5AB]/50 rounded-lg transition"
                title="Favorite"
              >
                <svg
                  className={`w-4 h-4 ${recipe.isFavorite ? "fill-amber-400 text-amber-400" : "fill-none text-[#628141]/50"}`}
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path
            ...
            {recipe.isFavorite && (
              <span className="absolute top-5 right-5 text-amber-400 text-lg group-hover:hidden transition-opacity">
                ⭐
              </span>
            )}

            <h3 className="font-bold text-lg text-[#1B211A] line-clamp-2 pr-8">
              {recipe.title}
            </h3>
          ...
          <div className="bg-[#EBD5AB]/20 p-4 px-5 flex justify-between items-center text-xs font-bold text-[#1B211A]/60">
            <button
              ...
              className="cursor-pointer text-[#628141] hover:text-[#1B211A] transition"
            >
              View Details →
            </button>
            <button
              className="cursor-pointer text-red-800 hover:text-[#1B211A] transition"
              onClick={(e) => {
